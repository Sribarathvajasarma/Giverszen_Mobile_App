...
const AddUser = ({ navigation }) => {
    const [visible, setVisible] = useState(false);              //Create states to store data
    ...
    const map = useRef(1)
    const [data, setData] = useState({                              //Create object state to store values given by user
        username: '',
        phonenumber: '',
        longitude: '',
        latitude: '',
    })
    const [items, setItems] = useState([                           //Create dropdown values
        { label: '1Km', value: '1' },
        { label: '2Km', value: '2' },

    ])

    const handleNameChange = (val) => {                            //Function to handle name field changes
        setData({
            ...data,
            username: val,
        })
    ...
    const button1handler = () => {
        checkPermission();
        getLocation()
        setCoordinates(latlng)                 //Get device location and store it in latlng state
    }
    ...
    const handlesumit2 = () => {
        fetch("https://giverzenbackend.herokuapp.com/api/nonsmartphoneuser_register", {             //Post data in api 
            method: 'POST',
            headers: {
            ...
            .then(async (responseData) => {
                ...
                Alert.alert('User added succesfully')             //Show successfull message
            }).done();
    }
